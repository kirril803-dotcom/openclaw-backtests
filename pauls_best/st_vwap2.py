"""
Supertrend + Volume Weighted Average Price
"""
import pandas as pd
import numpy as np
import csv
...
# Buy: Price crosses above VWAP + ST bullish
df['Buy'] = (df['Close'] > df['VWAP']) & (df['Close'].shift(1) <= df['VWAP'].shift(1)) & (df['ST_dir'] == 'up')
df['Sell'] = (df['Close'] < df['VWAP']) & (df['Close'].shift(1) >= df['VWAP'].shift(1)) & (df['ST_dir'] == 'down')
