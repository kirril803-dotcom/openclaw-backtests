...
df['Chaikin'] = (df['CLV'] * df['Volume']).rolling(3).sum() - (df['CLV'] * df['Volume']).rolling(10).sum()
...
# Buy: Chaikin crosses 0 + ST bullish
df['Buy'] = (df['Chaikin'] > 0) & (df['Chaikin'].shift(1) <= 0) & (df['ST_dir'] == 'up')
df['Sell'] = (df['Chaikin'] < 0) & (df['Chaikin'].shift(1) >= 0) & (df['ST_dir'] == 'down')
