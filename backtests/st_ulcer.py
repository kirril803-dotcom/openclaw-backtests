...
# Ulcer Index
period = 14
df['PD'] = ((df['Close'] - df['Close'].rolling(period).max()) / df['Close'].rolling(period).max()) * 100
df['Ulcer'] = np.sqrt((df['PD']**2).rolling(period).mean())
...
# Buy: Ulcer < 5 (low volatility) + ST bullish
df['Buy'] = (df['Ulcer'] < 5) & (df['ST_dir'] == 'up') & (df['ST_dir'].shift(1) == 'down')
