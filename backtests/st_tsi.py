...
# Buy: TSI crosses above signal + ST bullish
df['Buy'] = (df['TSI'] > df['TSI_Signal']) & (df['TSI'].shift(1) <= df['TSI_Signal'].shift(1)) & (df['ST_dir'] == 'up')
df['Sell'] = (df['TSI'] < df['TSI_Signal']) & (df['TSI'].shift(1) >= df['TSI_Signal'].shift(1)) & (df['ST_dir'] == 'down')
