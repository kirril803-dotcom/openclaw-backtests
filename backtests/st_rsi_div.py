...
df['PriceLow'] = (df['Close'] < df['Close'].shift(1)) & (df['Close'] < df['Close'].shift(-1))
df['RSILow'] = (df['RSI'] < df['RSI'].shift(1)) & (df['RSI'] < df['RSI'].shift(-1))
df['Divergence'] = df['PriceLow'] & df['RSILow']
...
# Buy: RSI divergence + ST bullish
df['Buy'] = df['Divergence'] & (df['ST_dir'] == 'up') & (df['ST_dir'].shift(1) == 'down')
