...
df['StochRSI'] = 100 * (rsi - low_rsi) / (high_rsi - low_rsi)
...
# Buy: StochRSI < 20 + ST bullish
df['Buy'] = (df['StochRSI'] < 20) & (df['ST_dir'] == 'up') & (df['ST_dir'].shift(1) == 'down')
df['Sell'] = (df['StochRSI'] > 80) & (df['ST_dir'] == 'down') & (df['ST_dir'].shift(1) == 'up')
