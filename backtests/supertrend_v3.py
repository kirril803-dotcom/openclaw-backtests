...
for i in range(1, len(df)):
    ...
    if df.iloc[i]['ST_dir'] == 'up':
        df.loc[df.index[i], 'ST'] = df.iloc[i]['Lower']
    else:
        df.loc[df.index[i], 'ST'] = df.iloc[i]['Upper']

# Buy when ST direction changes from down to up
df['Buy'] = (df['ST_dir'] == 'up') & (df['ST_dir'].shift(1) == 'down')
