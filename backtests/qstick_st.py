...
df['QStick_SMA'] = df['QStick'].rolling(3).mean()
...
# Buy: QStick bullish + ST bullish
df['Buy'] = (df['QStick'] > 0) & (df['QStick'].shift(1) <= 0) & (df['ST_dir'] == 'up')
df['Sell'] = (df['QStick'] < 0) & (df['QStick'].shift(1) >= 0) & (df['ST_dir'] == 'down')
