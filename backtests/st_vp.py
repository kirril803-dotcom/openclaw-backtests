...
def volume_profile(df, lookback=20):
    ...
    for i in range(len(df)):
        if i < lookback:
            ...
            poc = window.groupby('Close')['Volume'].sum().idxmax()
            pocs.append(poc)
    return pocs

df['POC'] = volume_profile(df)

# Buy: ST bullish + price above POC
df['Buy'] = (df['ST_dir'] == 'up') & (df['ST_dir'].shift(1) == 'down') & (df['Close'] > df['POC'])
