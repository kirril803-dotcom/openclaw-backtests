...
df['Down'] = df['Change'].clip(upper=0).abs()
df['SumUp'] = df['Up'].rolling(period).sum()
df['SumDown'] = df['Down'].rolling(period).sum()
df['CMO'] = ((df['SumUp'] - df['SumDown']) / (df['SumUp'] + df['SumDown'])) * 100
...
# Buy: CMO < -50 + crosses up + ST bullish
df['Buy'] = (df['CMO'] > -50) & (df['CMO'].shift(1) <= -50) & (df['ST_dir'] == 'up')
df['Sell'] = (df['CMO'] < 50) & (df['CMO'].shift(1) >= 50) & (df['ST_dir'] == 'down')
