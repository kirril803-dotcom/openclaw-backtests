...
# Williams %R
period = 14
df['WR'] = -100 * (df['High'].rolling(period).max() - df['Close']) / (df['High'].rolling(period).max() - df['Low'].rolling(period).min())
...
# Buy: WR < -80 + crosses up + ST bullish
df['Buy'] = (df['WR'] > -80) & (df['WR'].shift(1) <= -80) & (df['ST_dir'] == 'up')
df['Sell'] = (df['WR'] < -20) & (df['WR'].shift(1) >= -20) & (df['ST_dir'] == 'down')
