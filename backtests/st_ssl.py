...
# SSL Channel
df['SMA_High'] = df['High'].rolling(20).mean()
df['SMA_Low'] = df['Low'].rolling(20).mean()
...
# Buy: Close > SMA_High + ST bullish
df['Buy'] = (df['Close'] > df['SMA_High']) & (df['ST_dir'] == 'up') & (df['ST_dir'].shift(1) == 'down')
df['Sell'] = (df['Close'] < df['SMA_Low']) & (df['ST_dir'] == 'down') & (df['ST_dir'].shift(1) == 'up')
