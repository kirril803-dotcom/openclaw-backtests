...
df['ROC_MA'] = df['ROC'].rolling(9).mean()
...
# Buy: ROC crosses above 0 + ST bullish
df['Buy'] = (df['ROC'] > 0) & (df['ROC'].shift(1) <= 0) & (df['ST_dir'] == 'up')
df['Sell'] = (df['ROC'] < 0) & (df['ROC'].shift(1) >= 0) & (df['ST_dir'] == 'down')
