...
period = 20
x = np.arange(period)
df['Trend'] = df['Close'].rolling(period).apply(lambda y: np.polyfit(x, y, 1)[0] * period/2 + np.polyfit(x, y, 1)[1], raw=True)
...
# Buy: Price above trend + ST bullish
df['Buy'] = (df['Close'] > df['Trend']) & (df['ST_dir'] == 'up') & (df['ST_dir'].shift(1) == 'down')
df['Sell'] = (df['Close'] < df['Trend']) & (df['ST_dir'] == 'down') & (df['ST_dir'].shift(1) == 'up')
