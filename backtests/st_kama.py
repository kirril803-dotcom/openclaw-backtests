...
def kama(series, period):
    fast = 2 / (2 + 1)
    slow = 2 / (30 + 1)
    diff = abs(series.diff(period))
    vol = series.diff().abs().rolling(period).sum()
    er = diff / vol
...
# Buy: Price > KAMA + ST bullish
df['Buy'] = (df['Close'] > df['KAMA']) & (df['ST_dir'] == 'up') & (df['ST_dir'].shift(1) == 'down')
df['Sell'] = (df['Close'] < df['KAMA']) & (df['ST_dir'] == 'down') & (df['ST_dir'].shift(1) == 'up')
